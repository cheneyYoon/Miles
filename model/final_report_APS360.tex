...
\textbf{Limitations \& Future Work:} (1) Visual features (ResNet-50) implemented but not trained due to 3\(\times\) training time—could add 3-5\% AUROC~\cite{ofli2020}; (2) English-only excludes 78\% of original dataset; (3) SHAP for instance-level explanations planned; (4) Deployed production system enables future A/B testing with real creators (system live at \texttt{cheneyyoon-miles-inference.hf.space}).
...
\textbf{Quality:} Model performance significantly exceeds targets (AUROC +14\%, velocity MAE 10\(\times\) better) and baseline (+75\%). Feature ablation provides interpretable insights (\(-10.5\%/-7.5\%/-3.5\%\)). Perfect test set generalization (0\% degradation) demonstrates robust learning beyond memorization.

\textbf{Production Deployment (Beyond Requirements):} We deployed a complete end-to-end system demonstrating real-world applicability:

\textbf{System Architecture:}
\begin{itemize}
\item \textbf{Frontend}: Next.js app on Vercel for user interaction (topic/vibe input)
\item \textbf{Backend}: Supabase (Postgres + Edge Functions) with Row Level Security, 5 tables, 14 performance indexes
\item \textbf{Data Pipeline}: Automated YouTube scraper using yt-dlp + GitHub Actions CRON (every 6 hours). \textbf{Result}: 72 videos scraped with 100\% success rate.
\item \textbf{ML Inference}: FastAPI service deployed on HuggingFace Spaces (\texttt{cheneyyoon-miles-inference}). Handles BERT preprocessing, ResNet image features, 18 scalar features. Inference: 300-500ms warm, 30-60s cold start (free tier CPU).
\item \textbf{Cost}: \$0/month using free tiers (Supabase, HuggingFace Spaces, GitHub Actions, Vercel)
\end{itemize}

\textbf{Technical Achievements:}
\begin{itemize}
\item Dockerized PyTorch inference with 422MB model checkpoint handling
\item Multi-task learning (classification + regression)
\item Systematic feature ablation for interpretability
\item Production ML practices: MLflow tracking, FP16 mixed precision (40\% speedup), early stopping, gradient clipping
\item Real-time data pipeline: scraper \(\to\) inference \(\to\) database in <2 minutes
\item Solved deployment challenges: checkpoint dict loading, Python 3.11 compatibility, CORS configuration
\end{itemize}

\textbf{Learning beyond requirements:} (1) Full-stack deployment (frontend, backend, ML inference); (2) Production infrastructure setup; (3) Automated data pipelines; (4) Docker containerization; (5) CI/CD with GitHub Actions; (6) Real user-facing application beyond academic proof-of-concept.
